...
<body onFocus="parent_disable();" onclick="parent_disable();">
	...
	<div id="wide">
		<input type="button" value="Load" id="upload-input" /> <input
			type="button" value="Source" id="source-node" /> <input
			type="button" value="Destination" id="dest-node" /> <input
			type="button" value="Rescue Centre" id="rescue" />
	</div>
		...
		<script>
			document.onload = (function(d3, saveAs, Blob, undefined) {
					...
					function reset(){
						...
						<%connectionString cs = new connectionString();
							...
							if (connection != null) {
								...
								String q1 = "Select * from node where building_name = 'lib'";/* session.getAttribute("building_name") */

								// add floor number also in the query

								rs = (s.executeQuery(q1));
								String q2 = "Select * from edge where building_name = 'lib'";/* session.getAttribute("building_name") */
				...
				<%
									...
									ResultSet rt = null;
									String q3 = "Select * from evacrun_path where building_name = 'lib' and scenario_name='scene1' and algorithm='CCRP_extended'";/* session.getAttribute("building_name") */

									// add floor number also in the query

									rt = (st.executeQuery(q3));
									while (rt.next()) {
										String initial = rt.getString("edgelist");
										String init = rt.getString("path");
										//			System.out.println(initial);
										int n = initial.length();

										String x = initial.substring(1, n - 1);
						...
						var k = "<%=y[i]%>";
						 d3.select("#"+k).style("stroke","black");
				<%}

										for (int i = 0; i < z.length; i++) {
											//System.out.println("#"+y[i]);%>
					var kw = "<%=z[i]%>";
					 d3.select("#"+kw).style("fill","black");
				<%}
									}
					...
					var dragSvg = d3.behavior.zoom().on("zoom", function() {
						if (d3.event.sourceEvent.shiftKey && canEdit) {
...
d3	.select("#source-node")
		...
		<%if (connection != null) {
					...
					ResultSet rss = null;
					// the specific names of building and scenarios are to be given
					String q1s = "Select * from scenario_node where building_name = 'lib' and scenario_name='scene1'";/* session.getAttribute("building_name") */

					// add floor number also in the query

					rss = (ss.executeQuery(q1s));

					while (rss.next()) {
						String id = rss.getString("node_id");
						String type = rss.getString("node_type");
						//	System.out.println(id + " " + x + " " + y);%>
				     var iddd = "<%=id%>";
				     var typeee = "<%=type%>";
				 //    d3.select("#"+iddd).style("fill","gray").attr("r",nodeRadius);
			//	    alert(iddd +" "+typeee);
				  <%if (type.compareTo("source") == 0) {%>
						d3.select("#"+iddd).style("fill","red").attr("r",20);
					<%}
					}
				}else{
				   System.out.println("Not connected");
		...
		<%if (connection != null) {
					...
					ResultSet rsq = null;
					// the specific names of building and scenarios are to be given
					String q1q = "Select * from scenario_node where building_name = 'lib' and scenario_name='scene1'";/* session.getAttribute("building_name") */

					// add floor number also in the query

					rsq = (sq.executeQuery(q1q));

					while (rsq.next()) {
						String id = rsq.getString("node_id");
						String type = rsq.getString("node_type");
						//	System.out.println(id + " " + x + " " + y);%>
				     var iddd = "<%=id%>";
				     var typeee = "<%=type%>";
				 //    d3.select("#"+iddd).style("fill","gray").attr("r",nodeRadius);
			//	    alert(iddd +" "+typeee);
				  <%if (type.compareTo("destination") == 0) {%>
						d3.select("#"+iddd).style("fill","green").attr("r",20);
		...
		<%if (connection != null) {
					...
					ResultSet rsw = null;
					// the specific names of building and scenarios are to be given
					String q1w = "Select * from scenario_node where building_name = 'lib' and scenario_name='scene1'";/* session.getAttribute("building_name") */

					// add floor number also in the query

					rsw = (sw.executeQuery(q1w));

					while (rsw.next()) {

						String id = rsw.getString("node_id");
						String type = rsw.getString("node_type");%>
				     var iddd = "<%=id%>";
				     var typeee = "<%=type%>";
										//     d3.select("#"+iddd).style("fill","gray").attr("r",nodeRadius);
										//	alert(iddd + " " + typeee)
		<%if (type.compareTo("rescueCenter") == 0) {
		
		%>
			//alert(iddd +" "+typeee);
										d3.select("#" + iddd).style("fill",
												"blue").attr("r", 25);
			...
			<%		}
				...
				GraphCreator.prototype.circleMouseClick = function(d3node, d) {
					...
					state.mouseDownNode = d;
					document
							.write("< a	href = 'NodeForm.jsp?param_id' + d.id > </a>")
					//	alert("entered")
					nodechild_open(d.id, d.x, d.y);
				...
				GraphCreator.prototype.svgMouseUp = function() {
					...
					if (state.justScaleTransGraph) {
						...
						thisGraph.selectElementContents(txtNode);
						txtNode.focus();
					} else if (state.shiftNodeDrag && canEdit) {
						// dragged from node
						state.shiftNodeDrag = false;
						thisGraph.dragLine.classed("hidden", true);
					}
				...
				GraphCreator.prototype.updateGraph = function() {
					...
					newGs.classed(consts.circleGClass, true).attr("transform",
						...
						if (state.shiftNodeDrag && canEdit) {
