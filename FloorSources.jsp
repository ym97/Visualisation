...
var NodeList=null ,EdgeList=null;
var loadDone = false;
</script>
...
<body onFocus="parent_disable();" onclick="parent_disable();">
	...
	<div id="wide">
		<input type="button" value="Load" id="upload-input" />
	</div>
   ...
   <% String para = request.getParameter("param1");
      String temp [] = para.split("__");
      String flr = temp[1];
      String list = temp[0];
   %>
	<div id="plan" style="width: 700px; margin: 0 auto;">
		...
		<script>
			...
			if (connection != null) {
				...
				String q1 = "Select * from node where building_name = 'lib'and floor="+flr;/* session.getAttribute("building_name") */

				// add floor number also in the query

				rs = (s.executeQuery(q1));
				String q2 = "Select * from edge where building_name = 'lib' and floor ="+flr;/* session.getAttribute("building_name") */
		...
		<%}
					...
					String my_list[] = list.split(",");
					
					for(int w=0;w<my_list.length;w++){
					    	  %>
					    	    var ppp ="<%=my_list[w]%>";
					    	    d3.select("#"+ppp).style("fill","red").style("r",nodeRadius*1.4);
					    	  <%
					}
				...
				function call(id) {
					...
					var i, temp = [], temps = [];
					 ...
					 popupPath = window
							.open(
									"ForPath.jsp?param=" + id,
									"_blank",
									"visible=none,directories=no,titlebar=no,toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=1000,height=800,left=20,top=0")
					}
				...
				GraphCreator.prototype.pathMouseClick = function(d3path, d) {
					...
					//   TODO graph function to be inserted.
					///  
					//////////////
				};
				...
				GraphCreator.prototype.updateGraph = function() {
					...
					paths.enter().append("path").style('marker-end',
						...
						state.mouseDownLink = null;
							...
							"click",
							function() {
								thisGraph.pathClick.call(thisGraph, d3
										.select(this), d);
							});
					...
					newGs.classed(consts.circleGClass, true).attr("transform",
						...
						d3.select(this).classed(consts.connectClass, false);
							...
							function() {
								...
								thisGraph.circleClick.call(thisGraph, d3
										.select(this), d);
							})
